...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
       ...
       elseif ($_GET['url'] == "get_details") {
              if (@$token = $_COOKIE['Passwordhash']) {
              if (@$LoggedIn=$db->query('SELECT user_id FROM login_tokens WHERE token=:token', array(':token'=>sha1($token)))[0]['user_id']) {                   
               }else{
                     echo json_encode(array('status'=>'Not logged in','msg'=>0));
                     die();
                    }
        }else{
                     echo json_encode(array('status'=>'Not logged in','msg'=>0));
                  ...
                  $query=$db->query('SELECT id as password_id,user_id,site_name,token,datee  FROM pwd_details WHERE user_id=:id ORDER by id DESC',array(':id'=>$loggedinUser));
                   echo json_encode($query);
            
    }elseif ($_GET['url'] == "logged") {
        if (@$token = $_COOKIE['Passwordhash']) {
              if (@$LoggedIn=$db->query('SELECT user_id FROM login_tokens WHERE token=:token', array(':token'=>sha1($token)))[0]['user_id']) {                   
               }else{
                     echo json_encode(array('status'=>'Not logged in','msg'=>0));
                   ...
                   $token = $_COOKIE['Passwordhash'];
                   $loggedinUser = $db->query('SELECT user_id FROM login_tokens WHERE token=:token', array(':token'=>sha1($token)))[0]['user_id'];
                   $name = $db->query('SELECT Fullname FROM users WHERE id=:token', array(':token'=>$loggedinUser))[0]['Fullname'];
                  
                    echo json_encode(array('name'=>$name,'msg'=>'Logged in'));
                  ...
                  if ($_GET['url'] == "signup") {
                      ...
                      check_if_empty($email);
                            ...
                            if (strlen($password) <= 7 OR strlen($password) >= 50) {
                          ...
                          $db->query('INSERT INTO users VALUES (\'\',:fullname,:username,:email,:password)', 
                            array(':fullname'=>$fn,':username'=>$username, ':email'=>$email, 
                            ':password'=>$password) );
        ...
        check_if_empty($password);                            
          ...
          $password = preg_replace('/\s+/', '', $password);
           if ($db->query('SELECT username FROM users WHERE  email=:username',array(':username'=>$username))) {
                if (password_verify($password,  $db->query('SELECT password FROM users WHERE email=:username',
                    array(':username'=>$username))[0]['password'])){
                        $cstrong=True;
                        $token=bin2hex(openssl_random_pseudo_bytes(64, $cstrong));
                        $user_id=$db->query('SELECT id FROM users WHERE email=:username',
             ...
             die();
                        ...
                        $postBody = json_decode($postBody);
                  echo $postid = $postBody->userId;
                   $token = $_COOKIE['Passwordhash'];
                   $loggedinUser = $db->query('SELECT user_id FROM login_tokens WHERE token=:token', array(':token'=>sha1($token)))[0]['user_id'];  

                
                 $db->query('DELETE FROM pwd_details WHERE id=:postid AND user_id=:id', array(':postid'=>$postid,':id'=>$loggedinUser));

               //  $db->query('UPDATE users SET no_request=no_request-1 WHERE id=:postid', array(':postid'=>$loggedinUser));
